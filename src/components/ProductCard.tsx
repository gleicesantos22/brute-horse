...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className={`relative ${getCardBackground()} rounded-3xl p-8 border-3 transition-all duration-500 ${getHoverEffects()} ${borderColors[borderColor]} group h-full flex flex-col overflow-hidden`}>
      ...
      {/* Enhanced Badge */}
      <div className={`absolute ${getBadgePosition()} left-1/2 transform -translate-x-1/2 z-20`}>
        <div className={`${badgeColors[badge.color]} text-white ${getBadgePadding()} rounded-full ${getBadgeFont()} transition-all duration-300 group-hover:shadow-2xl ${borderColor === 'yellow' ? 'animate-pulse' : ''}`}>
          {badge.text}
      ...
      {/* Enhanced Header */}
      <div className={`text-center mb-8 ${getTopPadding()}`}>
